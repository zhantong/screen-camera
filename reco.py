...
def black_white(img, pixels):
	...
	for y in range(1,5):
		row=height*y/5
		for line in range(int(width/5),int(width*4/5)):
			sample[pixels[line,row]]+=1
	first_peak=sample.index(max(sample))
	print(first_peak)
	max_p=0
	second_peak=0
	for index,value in enumerate(sample):
		point=value*(first_peak-index)*(first_peak-index)
		if point>max_p:
			max_p=point
			second_peak=index
	print(second_peak)
	...
	best_v=0
	best_valley=0
	for x in range(first_peak,second_peak):
		point=(x-first_peak)*(second_peak-x)*(second_peak-x)*(sample[first_peak]-sample[x])
		if point>best_v:
			best_v=point
			best_valley=x
	print(best_valley)
